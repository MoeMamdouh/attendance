...
class App extends React.Component {
	...
	render() {
		...
		console.log('RENDEER: persons ', persons, 'departments ', departments, "records ", records)
		//if data exist
			...
			return (
				<LocaleProvider locale={enUS}>
					<div style={{ width: "80%", margin: '100px auto' }}>
						<InputGroup compact>
							<DatePicker onChange={value => this.setState({ startDate: value.toISOString() })} />
							<DatePicker onChange={value => this.setState({ endDate: value.toISOString() })} />
						...
						<div style={{ marginTop: 20 }}>Start Date: {this.state.startDate.toString()}</div>
						<div style={{ marginTop: 20 }}>End Date: {this.state.endDate.toString()}</div>

						<UsersList records={records} />
						<Charts />
					</div>
				</LocaleProvider>
			);
